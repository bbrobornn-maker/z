...
export default function GraphPage() {
  ...
  const vaultId = searchParams.get("vaultId") ? parseInt(searchParams.get("vaultId")!, 10) : undefined;
  const { data: graph, isLoading } = useGetGraph(vaultId ? { query: { vaultId } } : undefined);
  ...
  useEffect(() => {
    ...
    const step = () => {
      for (const node of nodes) {
        ...
        node.vx += (toCenterX / dist) * 0.05;
        node.vy += (toCenterY / dist) * 0.05;
      }
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dragRef.current.nodeId !== nodes[i].id) {
            nodes[i].vx -= fx;
            nodes[i].vy -= fy;
          }
          if (dragRef.current.nodeId !== nodes[j].id) {
            nodes[j].vx += fx;
            nodes[j].vy += fy;
          }
        }
      ...
      for (const edge of edgesRef.current) {
        ...
        const fx = (dx / dist) * force;
        const fy = (dy / dist) * force;
        if (dragRef.current.nodeId !== source.id) {
          source.vx += fx;
          source.vy += fy;
        }
        if (dragRef.current.nodeId !== target.id) {
          target.vx -= fx;
          target.vy -= fy;
        }
      }
